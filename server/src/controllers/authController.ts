...
const dbUser = prisma.user as any;
...
export const login = async (req: Request, res: Response) => {
  try {
    ...
    const cleanEmail = String(email).trim().toLowerCase();
    const user: any = await dbUser.findUnique({ where: { email: cleanEmail } });

    if (!user) {
      // Fake bcrypt comparison to protect against timing attacks
      await bcrypt.compare(password, '$2a$10$abcdefghijklmnopqrstuuNOPQRSTUVWXYZabcdefghijklmnopqr');
...
export const getMe = async (req: Request, res: Response) => {
  try {
    ...
    const user: any = await dbUser.findUnique({
      where: { id: userId },
    });
...
export const changePassword = async (req: Request, res: Response) => {
  try {
    ...
    const user: any = await dbUser.findUnique({ where: { id: userId } });
    if (!user) {
      return res.status(404).json({ success: false, message: 'User not found.' });
    }
...
export const facilityRegister = async (req: Request, res: Response) => {
  try {
    const {
      name, email, password, phone,
      facilityName, address, city, state, pincode,
      latitude, longitude, facilityPhone, facilityEmail,
      website, openingHours, acceptedCategories, recyclingServices, capacity
    } = req.body;
...
export const facilityLogin = async (req: Request, res: Response) => {
  try {
    const { email, password } = req.body;
