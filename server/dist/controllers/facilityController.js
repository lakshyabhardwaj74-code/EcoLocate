...
exports.getManagedFacilityStats = exports.addReview = exports.updateFacility = exports.createFacility = exports.getFacilityById = exports.getFacilities = void 0;
...
const getFacilities = async (req, res) => {
    try {
        ...
        let facilities = await prisma_js_1.default.facility.findMany({
            where: whereClause,
            include: {
                reviews: {
                    select: { rating: true },
...
const getFacilityById = async (req, res) => {
    try {
        ...
        const facility = await prisma_js_1.default.facility.findUnique({
            where: { id },
            include: {
                reviews: {
                    include: {
...
const createFacility = async (req, res) => {
    try {
        const { name, address, city, state, pincode, latitude, longitude, phone, email, website, openingHours, acceptedCategories, recyclingServices, capacity, } = req.body;
        ...
        const facility = await prisma_js_1.default.facility.create({
            data: {
...
const updateFacility = async (req, res) => {
    try {
        const id = String(req.params.id);
        const existing = await prisma_js_1.default.facility.findUnique({ where: { id } });
        if (!existing) {
            return res.status(404).json({ success: false, message: 'Facility not found.' });
        }
        ...
        const updated = await prisma_js_1.default.facility.update({
            where: { id },
            data: updateData,
        });
...
const addReview = async (req, res) => {
    try {
        ...
        let review;
        if (existingReview) {
            review = await prisma_js_1.default.review.update({
            ...
            review = await prisma_js_1.default.review.create({
                data: {
                    userId: req.user.id,
                    facilityId: id,
                    rating: numericRating,
        ...
        const allReviews = await prisma_js_1.default.review.findMany({ where: { facilityId: id } });
        const avgRating = parseFloat((allReviews.reduce((acc, curr) => acc + curr.rating, 0) / allReviews.length).toFixed(1));
        await prisma_js_1.default.facility.update({
            where: { id },
...
const getManagedFacilityStats = async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ success: false, message: 'Authentication required.' });
        }
        const facility = await prisma_js_1.default.facility.findFirst({
        ...
        const recentRequests = await prisma_js_1.default.pickupRequest.findMany({
            where: { facilityId: facility.id },
            take: 5,
            orderBy: { createdAt: 'desc' },
            select: {
...
exports.getManagedFacilityStats = getManagedFacilityStats;
